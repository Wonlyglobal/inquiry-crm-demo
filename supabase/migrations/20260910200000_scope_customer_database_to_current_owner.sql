-- Salespeople may retain access only while they currently own an inquiry.
-- A freshly created orphan company/contact stays visible to its creator long
-- enough for the existing company -> contact -> inquiry creation flow to finish.
...
using (
  ...
  or exists(
    ...
    where i.company_id=companies.id and i.owner_id=(select auth.uid())
  )
  or (
    created_by=(select auth.uid())
    and not exists(select 1 from public.inquiries i where i.company_id=companies.id)
  )
);
...
using (
  ...
  or exists(
    select 1 from public.inquiries i
    where i.company_id=contacts.company_id and i.owner_id=(select auth.uid())
  )
  or (
    created_by=(select auth.uid())
    and not exists(select 1 from public.inquiries i where i.company_id=contacts.company_id)
  )
);
    ...
    where c.id=contacts.company_id
      and (
        ...
        or (
          ...
          and not exists(select 1 from public.inquiries i where i.company_id=c.id)
        )
      )
  )
);
...
begin
  ...
  insert into public.contacts(
    ...
    target_company_id,trim(contact_name),normalized_email,
    nullif(trim(contact_phone),''),nullif(trim(contact_whatsapp),''),
    nullif(trim(contact_job_title),''),nullif(trim(contact_linkedin_url),''),actor_id
  ) returning * into saved;

  insert into public.audit_logs(
    actor_id,entity_type,entity_id,action,before_data,after_data,reason
  ) values (
    actor_id,'contact',saved.id,'contact_created','{}'::jsonb,
    jsonb_build_object(
      'company_id',saved.company_id,'full_name',saved.full_name,'email',saved.email,
      'phone',saved.phone,'whatsapp',saved.whatsapp,'job_title',saved.job_title,
