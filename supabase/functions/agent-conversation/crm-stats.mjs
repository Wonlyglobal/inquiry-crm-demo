const SOURCES=new Set('website email feishu google_ads meta_ads linkedin exhibition self_developed referral other'.split(' '));
const STAGES=new Set('pending_assignment received qualified contacted quoted sample_sent negotiating won lost'.split(' '));
const COUNTRIES=new Set('KZ MX BR US AE OM SA QA UZ KW BH NG IN GH ID VN MY PH GE AZ KG EG TH ET TZ BD DZ KE PK LK ZA MM MA KH RU PE CO TR PL AF CL EC RO TJ MV UA LA CI JO LB UG MZ MN SN CM AO NP IQ TM CN GB DE FR AU CA'.split(' '));
export function summarizeCrm(rows,{start,end},now=Date.now()){
 ...
 const group=(key,allowed)=>{const counts={};for(const r of safe){const raw=key==='source'?(r.primary_source||r.source):r[key];const label=allowed.has(raw)?raw:'other_or_unknown';counts[label]=(counts[label]||0)+1}return Object.entries(counts).filter(([,count])=>count>=5).map(([label,count])=>({label,count}))};
