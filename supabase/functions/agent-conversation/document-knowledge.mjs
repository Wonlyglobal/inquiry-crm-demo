...
export function documentEvidence(d){
 ...
 return `文档：${labels[d.status]||labels.not_indexed}；已处理 ${d.pages_processed}/${d.pages_total} 页，有文字 ${d.pages_with_text} 页。\n`+d.pages.map(p=>`[${p.location}｜${p.kind==='sheet_cells'?'单元格记录':p.kind==='ocr'?'OCR文字，需核对':'原文及OCR，需核对'}]\n${p.chunks.join('\n')}\n${p.facts.map(f=>`资料中的${f.field}表述（待核验）：${f.quote}`).join('\n')}`).join('\n')+'\n页码基于内部解析版本；参数、认证、适用条件以原文为准，不将其他型号或历史版本作为当前产品结论。';
}
