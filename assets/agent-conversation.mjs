...
export function mountConversation(host,{invoke,getPersona,onMessage,onMode,onTranscript,isAllowed}){
 ...
 async function record(){
  ...
  if(!ready||busy||mode.value!=='openai')return;
  stopAll();const epoch=version,persona=getPersona();busy=true;state('等待麦克风授权…');
  try{
   if(!navigator.mediaDevices?.getUserMedia||!window.MediaRecorder)throw Error('此浏览器不支持录音，请使用最新版Chrome或Safari');
   ...
   const mime=['audio/webm;codecs=opus','audio/mp4','audio/ogg;codecs=opus'].find(t=>MediaRecorder.isTypeSupported(t));if(!mime)throw Error('此浏览器没有受支持的录音格式');
