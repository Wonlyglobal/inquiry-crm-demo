...
## 2026-09-20 风险审查中心阶段 A（生产已发布）

- 已新增风险案件、仅追加事件链、账号敏感能力控制、P0–P3 SLA、角色范围与人工处置 RPC。
- 已新增确定性业务风险扫描：有效询盘无人负责、首次响应逾期、跟进逾期超过 24 小时；重复风险合并，不自动修改询盘或客户归属。
- 老板拥有全公司风险终审，销售主管仅处理本团队业务风险；市场部与业务员没有风险调查入口，被调查人不能审查本人。
- 前端已新增风险审查中心及处置时间线，并让现有看板/AI 报告导出读取风险控制状态。统一服务端导出和真实会话撤销仍未建设，不能把当前前端阻断描述为完整强制控制。
- 本地全量自动化 285/285、模块语法与差异检查通过。生产迁移 `20260920090000_risk_review_center.sql` 已应用，小时级确定性扫描任务已启用。
- 生产事务回滚验收结果：`risk_cases=true; immutable_events=true; reviewed_rpc=true; anon_review_denied=false`；验收事务已回滚，没有留下测试风险案件、控制或审计数据。
- 前端风险审查中心已由提交 `15c5196` 发布到 `main`，GitHub Pages 工作流 `35482432790` 成功；线上 `index.html` 与发布文件 SHA-256 均为 `c7650355a422c2822e208d35a3013314ad85570764276d9d2d5d303268554c19`。真实会话撤销、统一服务端导出和外部防篡改归档仍未建设，当前阶段不得描述为完整零信任闭环。
...
## 2026-09-17 真实邮件 AI 分拣验收

- 生产 `email-intake-ai` 已改为关闭网关旧 JWT 校验、由函数内部 `auth.getUser()` 验证新 signing key 会话；前端仅对该 AI 请求将超时提高到 50 秒，覆盖服务端 40 秒模型超时。
- DeepSeek 生产调用真实返回 `Insufficient Balance`，因此没有冒充 DeepSeek 结果。已由 OpenAI Codex 对一封真实官网询价邮件生成结构化建议，以 `provider=openai-codex`、`model=gpt-5` 写入通用建议表。
- 结果为“真实询盘”，置信度 99%；提取了联系人、国家、经销商身份、1–50 件、1–3 个月采购周期、0.60×0.90 米智能窗与纱窗需求，并标注公司名和项目名缺失。七条引用证据均来自原邮件。
- CRM 邮件详情已实际显示建议、置信度、提取字段和原文证据；建议与 `ai_suggestion_generated` 审计日志已核对一致。未自动采纳、转询盘、删除邮件或联系客户。
- 自动化回归 219/219 通过；前端修复提交 `f00db46` 已经 GitHub Pages 构建 `35176045900` 成功部署。未完成项：DeepSeek 账户需充值或配置另一个可用提供方，否则页面“AI 分析”的自动函数调用仍会返回余额不足。
