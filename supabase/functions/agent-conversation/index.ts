import {analysisIntent,evidencePlan,deepAnalysisInstruction} from './deep-analysis.mjs';
...
Deno.serve(async req=>{
 ...
 try{
  ...
  if(input.action==='chat'){
   ...
   materialTurn=resolveMaterialTurn(input.question,input.materialHistory||[]);route=knowledgeRoute(materialTurn.question,input.history);if(route.materials){route.crm=route.seo=route.social=route.research=route.knowledge=false;route.publicQuery='';}const skipped={status:'not_requested'};
