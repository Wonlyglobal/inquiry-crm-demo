...
Deno.serve(withReadOnlyGuard(async req=>{
  ...
  try{
    ...
    const [salesResult,historyResult,reminderResult]=await Promise.all([
      admin.from("profiles").select("id,full_name,email,team,job_title").eq("role","sales").eq("active",true).order("full_name"),
    ...
    const dataError=salesResult.error||historyResult.error||reminderResult.error;if(dataError)throw dataError;
    ...
    const candidates=sales.map(person=>{
      ...
      const countryScore=Math.min(20,countryCount*5),productScore=Math.min(25,productCount*5),capacityScore=Math.max(0,25-open.length*3-pendingReplies*4),slaScore=slaRate===null?10:Math.round(slaRate*20),continuityScore=companyCount?10:0,total=Math.max(0,Math.min(100,countryScore+productScore+capacityScore+slaScore+continuityScore));
      const warnings=[] as string[];if(open.length>=8)warnings.push(`在手有效询盘较多（${open.length} 条）`);if(pendingReplies>=3)warnings.push(`客户待回复较多（${pendingReplies} 条）`);if(!country&&!product)warnings.push("询盘缺少国家和产品信息，匹配依据有限");
      return {sales_id:person.id,name:person.full_name,email:person.email,team:person.team||"",job_title:person.job_title||"",score:total,breakdown:{country:{score:countryScore,max:20,count:countryCount,label:country?`${inquiry.target_country} 历史经验`:`国家信息缺失`},product:{score:productScore,max:25,count:productCount,label:product?`${inquiry.product_category} 历史经验`:`产品信息缺失`},capacity:{score:capacityScore,max:25,open_inquiries:open.length,pending_replies:pendingReplies,label:"当前负荷"},sla:{score:slaScore,max:20,sample_size:eligible.length,on_time_count:onTime,rate:slaRate,label:"30 分钟首响"},continuity:{score:continuityScore,max:10,count:companyCount,label:"同客户连续性"}},warnings,last_activity_at:owned.map(item=>item.first_valid_contact_at||item.assigned_at||item.created_at).filter(Boolean).sort().at(-1)||null};
    }).sort((a,b)=>b.score-a.score||a.name.localeCompare(b.name,"zh-CN"));
    const top=candidates.slice(0,3),coverage=[Boolean(country),Boolean(product),history.length>0,sales.some(person=>history.some(item=>item.owner_id===person.id&&item.first_valid_contact_at))].filter(Boolean).length/4,confidence=Math.min(.9,.5+coverage*.4);
    const sourceHash=await sha256(JSON.stringify({schema_version:"assignment-v1",inquiry:{id:inquiry.id,country,product,company_id:inquiry.company_id,updated_at:inquiry.updated_at},candidates:candidates.map(item=>[item.sales_id,item.score,item.breakdown])}));
    ...
    const proposedData={recommended_sales_id:top[0]?.sales_id||null,recommendations:top,candidate_count:candidates.length,scoring_version:"assignment-v1",weights:{product:25,country:20,capacity:25,sla:20,continuity:10},guardrail:"仅供主管参考，不自动分配"},evidence=top.map(item=>({sales_id:item.sales_id,score:item.score,breakdown:item.breakdown,warnings:item.warnings}));
    const rationale=`依据产品经验、国家/地区经验、当前在手询盘、待回复数量、30 分钟首响表现和同客户连续性进行可解释评分。统计截止 ${new Date(now).toISOString()}，不使用黑箱成交概率，最终分配仍由主管确认。`;
    const {data:suggestionId,error:saveError}=await admin.rpc("record_ai_suggestion",{target_suggestion_type:"assignment_recommendation",target_target_type:"inquiry",target_target_id:inquiry.id,target_source_hash:sourceHash,target_provider:"rules",target_model:"assignment-score-v1",target_confidence:confidence,target_proposed_data:proposedData,target_evidence:evidence,target_rationale_zh:rationale,target_requested_by:user.id});
