...
test('opening Grace preloads a fixed greeting and starts local wake without sending recordings or chat',async()=>{
 ...
 try{await model.enter();assert.equal(model.isModel(),true);assert.equal(Recognition.instances.length,1);assert.equal(Recognition.instances[0].processLocally,true);assert.deepEqual(calls,['status','greeting'])}finally{model.reset()}
});
