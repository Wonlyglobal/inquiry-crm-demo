...
export function mountAgentWorld(host,{onSelect,onAnalysis,isAllowed}){
 ...
 root.querySelectorAll('[data-advisor]').forEach(button=>button.onclick=()=>{if(!isAllowed())return;const next=button.dataset.advisor;if(onSelect(next,root.querySelector('#world-chat-slot'))===false)return;selected=next;root.querySelector('#world-selection').textContent=selected+' · '+roles[selected];root.querySelector('#world-show-analysis').hidden=false;});
