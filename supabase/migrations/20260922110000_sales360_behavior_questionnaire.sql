...
begin
 ...
 for question in select * from jsonb_array_elements(questions) loop
  ...
  if jsonb_typeof(answer->'value') is distinct from 'number' then raise exception '请选择1到5分或未观察'; end if;
  value:=(answer->>'value')::numeric;
  if value<1 or value>5 or value<>trunc(value) then raise exception '选择题必须为1到5的整数'; end if;
  ...
  if value in (1,2,5) and char_length(btrim(coalesce(answer->>'evidence','')))<8 then raise exception '1、2或5分需要至少8字的逐题事例：%',question->>'text'; end if;
...
begin
  ...
  foreach dimension in array required_dimensions loop
    ...
    if score_value<1 or score_value>5 then raise exception '评分必须在 1 到 5 之间'; end if;
