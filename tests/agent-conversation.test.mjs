...
test('OpenAI access requires exact enabled existing world identity',()=>{
...
test('model body has role, bounded history, public evidence and no tool execution or response storage',()=>{
 const b=requestBody(q,'gpt-5.4-mini','PUBLIC SOURCES');assert.equal(b.store,false);assert.equal(b.tools,undefined);assert.match(b.instructions,/Grace/);assert.match(b.instructions,/不能声称已读取CRM/);assert.equal(b.input.length,1);
});
test('incomplete/empty model output is not shown as a successful answer',()=>{
 ...
 assert.equal(outputText({status:'completed',output:[{type:'message',content:[{type:'output_text',text:'明确标出假设。'}]}]}),'明确标出假设。');
});
test('existing customer-data containment stays fail-closed; new path requires server policy and audit',()=>{
 ...
 assert.match(old,/return assertCustomerDataAiPolicy\(\)/);assert.match(source,/OPENAI_AGENT_POLICY/);assert.match(source,/if\(!enabled\)return/);assert.match(source,/if\(auditError\)return/);assert.match(source,/data.user!==user.id/);assert.match(source,/data.persona!==input.persona/);assert.match(source,/data.expires<Date.now\(\)/);
