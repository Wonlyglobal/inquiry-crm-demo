...
begin
  if not exists(select 1 from public.inquiries where id=target_inquiry_id and (owner_id=auth.uid() or private.current_crm_role() in ('owner','sales_manager'))) then raise exception '只能为本人负责或有权管理的询盘创建报价'; end if;
